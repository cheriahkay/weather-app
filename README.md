# 🌤️ Modern Weather App

A sleek, responsive weather application built with Vanilla JavaScript that fetches real-time data from the OpenWeatherMap API.

## 🚀 Live Demo
Check out the live project here: [https://weather-app-steel-nu-20.vercel.app]

## ✨ Features
- **Real-time Data:** Fetches current temperature, humidity, and weather conditions.
- **Search Functionality:** Search for any city worldwide.
- **UX Improvements:** - Press "Enter" to search.
    - Input field clears automatically after search.
- **Modern Design:** Features a Dark Gradient tech-inspired UI.
- **Responsive:** Works perfectly on mobile, tablet, and desktop.

## 🛠️ Built With
- **HTML5** - Semantic structure
- **CSS3** - Custom Dark Gradient styling
- **JavaScript (ES6+)** - Fetch API & Async/Await logic
- **OpenWeatherMap API** - Weather data source
- **Vercel** - Hosting and Deployment



## ⚙️ How to Run Locally
1. Clone the repository:
   ```bash
   git clone [https://github.com/your-username/weather-app.git](https://github.com/your-username/weather-app.git)